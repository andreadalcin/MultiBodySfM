...
class SequenceFolder(data.Dataset):
    ...
    def crawl_folders(self, sequence_length):
        ...
        for scene in self.scenes:
            ...
            motions = np.genfromtxt(scene / 'motions.txt').astype(np.float32)
            ...
            for i in range(min(2, len(imgs))):
                ...
                motion_tgt = np.concatenate((motions[i, :].reshape((3, 4)), np.array([[0, 0, 0, 1]])), axis=0)
                sample = {'intrinsics': intrinsics, 'tgt': img, 'tgt_depth': depth, 'ref_imgs': [], 'ref_poses': [], 'ref_motions': []}
                for j in shifts:
                    ...
                    motion_src = np.concatenate((motions[j, :].reshape((3, 4)), np.array([[0, 0, 0, 1]])), axis=0)
                    motion_rel = motion_src @ np.linalg.inv(motion_tgt)
                    motion = motion_rel[:3, :].reshape((1, 3, 4)).astype(np.float32)

                    sample['ref_poses'].append(pose)
                    sample['ref_motions'].append(motion)
    ...
    def __getitem__(self, index):
        ...
        ref_motions = sample['ref_motions']
        ...
        return tgt_img, ref_imgs, ref_poses, ref_motions, intrinsics, np.linalg.inv(intrinsics), tgt_depth
