...
def main():
    ...
    with torch.no_grad():
        for ii, (tgt_img, ref_imgs, ref_poses, ref_motions, intrinsics, intrinsics_inv, tgt_depth) in enumerate(
                val_loader):
            if ii % args.print_freq == 0:
                ...
                ref_motions_var = [Variable(motion.cuda()) for motion in ref_motions]
                ...
                motion = torch.cat(ref_motions_var, 1)

                start = time.time()
                output_depth = dpsnet(tgt_img_var, ref_imgs_var, pose, motion, intrinsics_var, intrinsics_inv_var)
                ...
                if args.output_print:
                    output_disp_ = torch.squeeze(args.mindepth * args.nlabel / tgt_depth, 1)
